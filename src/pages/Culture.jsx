      ...
      <div className="flex-1 overflow-y-auto pb-20 bg-gray-50">
        ...
        <div className="relative h-36">
          ...
          <div className="absolute bottom-4 left-4">
            <h1 className="text-white font-bold text-lg">{tr('culture_title')}</h1>
          </div>
        </div>

        {/* Tabs */}
        <div className="bg-white px-4 py-2 flex gap-2 overflow-x-auto scrollbar-hide border-b border-gray-100">
          {Object.entries(sections).map(([key, s]) => (
            <button
              key={key}
              onClick={() => setActive(key)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold whitespace-nowrap transition-all ${
                active === key ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600'
              }`}
            >
              <span>{s.icon}</span>
              {s.title[lang]}
        ...
        <div className="px-4 py-4">
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-base font-bold text-gray-900">{sec.title[lang]}</h2>
            </div>
